...
.page-slider { grid-area: slider; }
.slider-restore-btn { grid-area: restore; }
...
const DocumentTypeSelector: React.FC<DocumentTypeSelectorProps> = ({ onChange }) => {
...
const ModelSelector: React.FC<ModelSelectorProps> = ({ 
  ...
  const models = [
    { id: 'smarteye' as AIModel, label: 'SmartEye', description: '문제지 특화' },
    { id: 'doclayout' as AIModel, label: 'DocLayout', description: '일반 문서' }
  ]
  ...
  return (
    <div className={styles.modelSelector}>
      <h3 className={styles.selectorTitle}>
        AI 모델
        {isAutoSelected && (
          <span className={styles.autoBadge}>자동 선택</span>
        )}
      </h3>
...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className={styles.sidebar}>
      <DocumentTypeSelector onChange={setDocumentType} />
      <ModelSelector selectedModel={selectedModel} isAutoSelected={isAutoSelected} />
...
const DownloadProgressModal: React.FC<DownloadProgressModalProps> = ({ 
  ...
  const modalContent = (
    <div className={styles.modalOverlay} onClick={onClose}>
      <div className={styles.progressModal} onClick={(e) => e.stopPropagation()}>
        <h3>다운로드 진행 중...</h3>
...
const IntegratedDownloadButton: React.FC<IntegratedDownloadButtonProps> = ({ pages }) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      const results = await downloadService.downloadAllPages(
        pages, 
        (p) => setProgress(p)
      )
...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className={styles.sidebar}>
      <DocumentTypeSelector onChange={setDocumentType} />
      <ModelSelector selectedModel={selectedModel} isAutoSelected={isAutoSelected} />
      <AnalyzeButton isLoading={isAnalyzing} hasFiles={hasFiles} onClick={handleAnalyze} />
...
export const useBoundingBox = (editorRef: RefObject<HTMLElement>) => {
  const scrollToEditor = useCallback((boxId: string) => {
    if (!editorRef.current) return
    
    const element = editorRef.current.querySelector(`[data-bbox-id="${boxId}"]`)
    ...
    <div className={styles.aiStatsTab}>
      ...
      <div className={styles.detailTable}>
        ...
        <table>
          ...
          <tbody>
            {Object.entries(analysisResult.classDistribution).map(([className, count]) => (
              <tr key={className}>
                <td>{className}</td>
                <td>{count}</td>
                <td>{(analysisResult.confidenceScores[className as keyof typeof analysisResult.confidenceScores] * 100).toFixed(1)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  switch (action.type) {
    case 'SET_DOCUMENT_TYPE':
      return {
        ...
        selectedModel: action.payload === 'worksheet' ? 'smarteye' : 'doclayout'
      }
    case 'SET_ANALYZING':
      return {
        ...state,
...
type PagesAction =
  ...
  | { type: 'UPDATE_PAGE_STATUS'; payload: { id: string; status: Page['analysisStatus'] } }
...
// src/components/sidebar/DocumentTypeSelector.tsx
const DocumentTypeSelector = React.memo<DocumentTypeSelectorProps>(({ onChange }) => {
  // ...
})
