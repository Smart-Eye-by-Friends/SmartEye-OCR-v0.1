...
class ProjectBase(BaseModel):
    ...
    project_name: str = Field(
        ..., min_length=1, max_length=255, description="프로젝트명"
    )
...
class QuestionGroupBase(BaseModel):
    ...
    page_id: int = Field(..., description="페이지 ID")
    anchor_element_id: int = Field(
        ..., description="앵커 요소 ID (question_number, question_type 등)"
    )
...
class QuestionElementBase(BaseModel):
    ...
    element_id: int = Field(..., description="요소 ID")
    order_in_question: int = Field(
        ..., ge=0, description="문제 내 요소 순서 (0부터 시작)"
    )
...
class CombinedResultBase(BaseModel):
    ...
    output_format: OutputFormatEnum = Field(
        OutputFormatEnum.DOCX, description="출력 형식"
    )
