...
public class PatternMatchingEngine {
     ...
     * ✅ Phase 3: 001, ★001 등의 패턴 추가
     */
    private static final List<Pattern> QUESTION_NUMBER_PATTERNS = Arrays.asList(
        // 🎯 고정밀도 패턴 (1순위)
        Pattern.compile("^\\s*[★□●◆■▲]?\\s*(\\d+)번\\s*"),      // "1번", "★1번", "□2번"
        Pattern.compile("^\\s*[★□●◆■▲]?\\s*(\\d+)\\.\\s*"),      // "1.", "★1." (특수 기호 포함)
        Pattern.compile("^\\s*Q\\s*(\\d+)\\s*"),    // "Q1", "Q2"
        Pattern.compile("^\\s*문제\\s*(\\d+)\\s*"), // "문제 1", "문제 2"
        Pattern.compile("^\\s*[★□●◆■▲]\\s*(\\d{3})\\s*"),  // ✅ "★001", "□002" (3자리 숫자)
