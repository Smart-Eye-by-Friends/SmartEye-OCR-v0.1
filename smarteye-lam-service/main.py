...
try:
    ...
    enhanced_analyzer = EnhancedLayoutAnalyzer()
    logger.info("✅ 구조화된 분석 모듈 로드 성공")
except ImportError as e:
    logger.warning(f"⚠️ 구조화된 분석 모듈 로드 실패: {e}")
...
async def analyze_structured_layout(
    image: UploadFile = File(...),
    model_choice: str = Form("SmartEyeSsen")
):
    """구조화된 문제별 레이아웃 분석 엔드포인트"""
    start_time = time.time()
    
    try:
        logger.info(f"구조화된 분석 요청 - 파일: {image.filename}, 모델: {model_choice}")
        
        # 구조화된 분석 모듈 확인
        if structured_generator is None:
            raise HTTPException(
                status_code=500, 
                detail="구조화된 분석 기능을 사용할 수 없습니다. 모듈 로드 실패."
            )
        ...
        try:
            ...
            logger.info(f"기본 레이아웃 분석 완료 - {len(layout_results)}개 요소 감지")
            ...
            for i, layout in enumerate(layout_results):
                class_name = layout.get('class', '').lower()
                ...
                try:
                    ...
                    if len(text) > 1:
                        ...
                        logger.debug(f"OCR 성공: {class_name} - '{text[:30]}...'")
                        
                except Exception as ocr_e:
                    logger.warning(f"OCR 실패: {ocr_e}")
                    continue
            
            logger.info(f"OCR 처리 완료 - {len(ocr_results)}개 텍스트 블록")
            
            # 3. 구조화된 분석 수행
            logger.info("🔧 구조화된 분석 수행 중...")
            ...
            for i, layout in enumerate(layout_results):
                ...
                layout_elements.append({
                    ...
                    'box': [int(bbox.get('x1', 0)), int(bbox.get('y1', 0)), 
            ...
            structured_result = structured_generator.generate_structured_json(
                ocr_results,
                [],  # AI 결과는 비어있음 (LAM 서비스에서는 OCR만 처리)
                layout_elements
            )
            ...
            response = {
                ...
                "device": device,
                "results": {
                    "basic_layout_analysis": layout_results,
            ...
            logger.info(f"✅ 구조화된 분석 완료 - 처리시간: {processing_time:.2f}초, " + 
                       f"문제 수: {structured_result.get('document_info', {}).get('total_questions', 0)}")
        ...
        logger.error(f"구조화된 분석 중 예상치 못한 오류: {e}")
        logger.error(f"상세 오류: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"구조화된 분석 서버 오류: {str(e)}")
